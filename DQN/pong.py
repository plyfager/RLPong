...
class Agent:
    ...
    def preProcess(self, image):
        ...
        if isinstance(image, tuple):
            frame = cv2.cvtColor(image[0], cv2.COLOR_BGR2GRAY)  # To grayscale
        else:
...
if __name__ == "__main__":
    ...
    for episode in range(startEpisode, MAX_EPISODE):
        ...
        for step in range(MAX_STEP):
            ...
            ttt = environment.step(action)
            next_state, reward, done, truncated, info = environment.step(action)  # Observe
